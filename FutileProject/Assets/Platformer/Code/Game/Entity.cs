...
namespace Platformer
{
	public class Entity : IEnumerable<Component>, IEnumerable
	{
		...
		{
			if (Collider != null)
				Collider.Render(Collidable ? Color.red : new Color(0.8f, 0, 0, 1.0f));
		}
		...
		public Entity Closest(params Entity[] entities)
		{
			...
			{
				float current = Vector2.Distance(Position, entities[i].Position);
				if (current < dist)
				{
					closest = entities[i];
					dist = current;
		...
		public Entity Closest(int tag)
		{
			...
			{
				closest = list[0];
				dist = Vector2.Distance(Position, closest.Position);

				for (int i = 1; i < list.Count; i++)
				{
					float current = Vector2.Distance(Position, list[i].Position);
					if (current < dist)
					{
						closest = list[i];
						dist = current;
