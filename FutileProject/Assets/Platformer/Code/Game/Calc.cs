...
namespace Platformer
{
	static public class Calc
	{
		...
		static private Stack<System.Random> randomStack = new Stack<System.Random>();

		static public void PushRandom(int newSeed)
		{
			randomStack.Push(Calc.Random);
			Calc.Random = new System.Random(newSeed);
		}

		static public void PushRandom(System.Random random)
		{
			randomStack.Push(Calc.Random);
			Calc.Random = random;
		}
		...
		static public T Choose<T>(this System.Random random, T a, T b)
		{
			return GiveMe<T>(random.Next(2), a, b);
		}

		static public T Choose<T>(this System.Random random, T a, T b, T c)
		{
			return GiveMe<T>(random.Next(3), a, b, c);
		}

		static public T Choose<T>(this System.Random random, T a, T b, T c, T d)
		{
			return GiveMe<T>(random.Next(4), a, b, c, d);
		}

		static public T Choose<T>(this System.Random random, T a, T b, T c, T d, T e)
		{
			return GiveMe<T>(random.Next(5), a, b, c, d, e);
		}

		static public T Choose<T>(this System.Random random, T a, T b, T c, T d, T e, T f)
		{
			return GiveMe<T>(random.Next(6), a, b, c, d, e, f);
		}

		static public T Choose<T>(this System.Random random, params T[] choices)
		{
		...
		/// <param name="max"></param>
		/// <returns></returns>
		/*static public Vector2 Range(this System.Random random, Vector2 min, Vector2 max)
		{
		...
		static public bool Chance(this System.Random random, float chance)
		{
			return random.NextFloat() < chance;
		}

		static public float NextFloat(this System.Random random)
		{
			return (float)random.NextDouble();
		}

		static public float NextFloat(this System.Random random, float max)
		{
			return random.NextFloat() * max;
		}

		static public float NextAngle(this System.Random random)
		{
		...
		static public void Shuffle<T>(this List<T> list, System.Random random)
		{
			int i = list.Count;
			int j;
			T t;
		...
		static public void ShuffleSetFirst<T>(this List<T> list, System.Random random, T first)
		{
			int amount = 0;
		...
		static public void ShuffleNotFirst<T>(this List<T> list, System.Random random, T notFirst)
		{
			int amount = 0;
		...
		{
			float ret = Mathf.Lerp(value1, value2, amount);
		...
		static public Vector2 LerpSnap(Vector2 value1, Vector2 value2, float amount, float snapThresholdSq = .1f)
		{
			...
			if ((ret - value2).sqrMagnitude < snapThresholdSq)
				return value2;
			else
				return ret;
		}
		...
		{
			return new Vector2((float)Mathf.Round(vec.x), (float)Mathf.Round(vec.y));
		}
		...
		{
			return (((angleRadians * Math.Sign(angleRadians) + (float)Math.PI) % ((float)Math.PI * 2)) - (float)Math.PI) * (float)Math.Sign(angleRadians);
		}
		...
		{
			return val + Mathf.Clamp(AngleDiff(val, target), -maxMove, maxMove);
		}
		...
		{
			return new Vector2(Mathf.Clamp(val.x, minX, maxX), Mathf.Clamp(val.y, minY, maxY));
		}
		...
		static public Vector2 FourWayNormal(this Vector2 vec)
		{
			...
			angle = (float)Math.Floor((angle + (float)Math.PI / 2.0f / 2f) / (float)Math.PI / 2.0f) * (float)Math.PI / 2.0f;
		...
		static public Vector2 EightWayNormal(this Vector2 vec)
		{
			...
			float angle = vec.Angle();
			angle = (float)Math.Floor((angle + (float)Math.PI / 4.0f / 2f) / (float)Math.PI / 4.0f) * (float)Math.PI / 4.0f;
		...
		{
			float divider = (float)Math.PI * 2 / slices;
		...
		{
			return AngleToVector(vec.Angle() + angleRadians, vec.magnitude);
		}

		static public Vector2 XComp(this Vector2 vec)
		{
			return new Vector2(1, 0) * vec.x;
		}

		static public Vector2 YComp(this Vector2 vec)
		{
			return new Vector2(0, 1) * vec.y;
		}
		...
		static public string IntGridToCSV(int[,] data)
		{
			...
			for (int y = 0; y < data.GetLength(1); y++)
			{
				...
				if (line.Count > 0)
				{
					string[] s = new string[line.Count];
					for(var i = 0 ; i < line.Count ; i++) {
						s[i] = line[i].ToString();
					}
					str.Append(String.Join(",", s));
					line.Clear();
				}
		...
		static public void SetAttr(this XmlElement xml, string attributeName, System.Object setTo)
		{
			XmlAttribute attr;
		...
		static public void SetChild(this XmlElement xml, string childName, System.Object setTo)
		{
			XmlElement ele;
		...
		static public void Dissect(System.Object obj)
		{
			ALTDebug.Log(obj.GetType().Name + " { ");
			foreach (var v in obj.GetType().GetFields())
				ALTDebug.Log(v.Name + ": " + v.GetValue(obj) + ", ");
			ALTDebug.Log(" }");
		}
		...
		static public Delegate GetMethod<T>(System.Object obj, string method) where T : class
		{
