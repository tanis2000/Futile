...
namespace Platformer
{
	public class MusicManager
	{
		...
		{
			if (isFadingOut)
				return; //we're already fading out!
			isFadingOut = true;
			...
			if (currentSong == null) {
				...
				Go.to (audioSource, fadeOutDuration, new GoTweenConfig ().floatProp ("volume", 0).onComplete (OnCurrentSongFadeOutComplete));
			}
		}
		...
		public AudioClip LoadAudioClip (string resourcePath)
		{
			...
			if (audioClip == null) {
				ALTDebug.Log ("MusicManager couldn't find music at: " + resourcePath);
