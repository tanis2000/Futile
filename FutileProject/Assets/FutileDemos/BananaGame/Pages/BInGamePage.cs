...
public class BInGamePage : BPage, FMultiTouchableInterface
{
	...
	override public void Start()
	{
		...
		Go.to(_scoreLabel, 0.5f, new GoTweenConfig().
			setDelay(0.0f).
			floatProp("alpha",1.0f));
		
		_timeLabel.alpha = 0.0f;
		Go.to(_timeLabel, 0.5f, new GoTweenConfig().
			setDelay(0.0f).
			floatProp("alpha",1.0f).
			setEaseType(GoEaseType.BackOut));
		
		_closeButton.scale = 0.0f;
		Go.to(_closeButton, 0.5f, new GoTweenConfig().
			setDelay(0.0f).
...
//		AddChild (_bananaContainer); 
	...
	private void CreateBananaExplodeEffect(BBanana banana)
	{
		...
		Go.to (explodeSprite, 0.3f, new GoTweenConfig().floatProp("scale",1.3f).floatProp("alpha",0.0f).onComplete(HandleExplodeSpriteComplete));
	}
