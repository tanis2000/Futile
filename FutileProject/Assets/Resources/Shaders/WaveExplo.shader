		...
		ZTest Always Cull Off ZWrite Off Fog { Mode Off } //Rendering settings
	 	...
	 	Pass{
			...
			fixed4 frag (v2f i) : COLOR{
				...
				//float2 uv_displaced = float2(i.uv.x + diff.x*cos(dist)*0.15/dist,i.uv.y + diff.y*cos(dist)*0.15/dist);
				//float2 uv_displaced = float2(i.uv.x - cos(dist*50)*diff.x*0.005/dist,i.uv.y - cos(dist*50)*diff.y*0.005/dist);
