		...
		ZTest Always Cull Off ZWrite Off Fog { Mode Off } //Rendering settings
	 	...
	 	Pass{
			...
			fixed4 frag (v2f i) : COLOR{
				...
				float diff=i.uv.x-_CenterX;
				float dist=abs(diff);
				//float2 uv_displaced = float2(i.uv.x + diff*cos(dist)*_Amplitude/dist,i.uv.y);
				...
				if (dist>_Radius) {
					if (dist<_Radius+wavesize) {
						...
						uv_displaced.x-=cossin*diff*_Amplitude/dist;
