...
public static class RXGoKitExtensions
{
	...
	//this makes it so we don't have to specify false for isRelative every.single.time.
	public static GoTweenConfig colorProp(this GoTweenConfig config, string propName, Color propValue)
	{
	...
	{
		config.onComplete((tween) => {((tween as GoTween).target as FNode).RemoveFromContainer();});	
		return config;
	}

	public static GoTweenConfig hideWhenComplete(this GoTweenConfig config)
	{
		config.onComplete((tween) => {((tween as GoTween).target as FNode).isVisible = false;});	
		return config;
	}

	//forward to an action with no arguments instead (ex you can pass myNode.RemoveFromContainer)
	public static GoTweenConfig onComplete(this GoTweenConfig config, Action onCompleteAction)
	{
		config.onComplete((tween) => {onCompleteAction();});	
		return config;
	}

	public static GoTweenConfig alpha(this GoTweenConfig config, float alpha)
	{
		config.tweenProperties.Add(new FloatTweenProperty("alpha",alpha,false));
		return config;
	}

	public static GoTweenConfig rotation(this GoTweenConfig config, float rotation)
	{
		config.tweenProperties.Add(new FloatTweenProperty("rotation",rotation,false));
		return config;
	}

	public static GoTweenConfig x(this GoTweenConfig config, float x)
	{
		config.tweenProperties.Add(new FloatTweenProperty("x",x,false));
		return config;
	}

	public static GoTweenConfig y(this GoTweenConfig config, float y)
	{
		config.tweenProperties.Add(new FloatTweenProperty("y",y,false));
		return config;
	}

	public static GoTweenConfig pos(this GoTweenConfig config, float x,float y)
	{
		config.tweenProperties.Add(new FloatTweenProperty("x",x,false));
		config.tweenProperties.Add(new FloatTweenProperty("y",y,false));
		return config;
	...
	public static GoTweenConfig pos(this GoTweenConfig config, Vector2 pos)
	{
	...
	public static GoTweenConfig scaleXY(this GoTweenConfig config, float scaleX,float scaleY)
	{
	...
	public static GoTweenConfig scaleXY(this GoTweenConfig config, float scale)
	{
		config.tweenProperties.Add(new FloatTweenProperty("scale",scale,false));
		return config;
	}

	public static GoTweenConfig backOut(this GoTweenConfig config)
	{
		config.easeType = GoEaseType.BackOut;
		return config;
	}

	public static GoTweenConfig backIn(this GoTweenConfig config)
	{
		config.easeType = GoEaseType.BackIn;
		return config;
	}

	public static GoTweenConfig expoOut(this GoTweenConfig config)
	{
		config.easeType = GoEaseType.ExpoOut;
		return config;
	}

	public static GoTweenConfig expoIn(this GoTweenConfig config)
	{
		config.easeType = GoEaseType.ExpoIn;
		return config;
	}

	public static GoTweenConfig expoInOut(this GoTweenConfig config)
	{
		config.easeType = GoEaseType.ExpoInOut;
		return config;
	}
