...
public class RXTweenable
{
	...
	{
		this.To(targetAmount,duration, new GoTweenConfig());
	}

	public void To(float targetAmount, float duration, GoTweenConfig tc)
	{
	...
	public static RXTweenable DelayAction(float delay, Action action)
	{
		...
		rt.To(1.0f,delay,new GoTweenConfig().onComplete((t)=>{action();}));
		return rt;
	}
}
...
{
	public List<GoTween> tweensToDo = new List<GoTween>();

	public RXTweenChain()
	{
	}

	public RXTweenChain Add(object target, float duration, GoTweenConfig config)
	{
		config.onComplete(HandleTweenComplete);
		GoTween tween = new GoTween(target,duration,config,null);
		tweensToDo.Add(tween);
		return this;
	}

	void HandleTweenComplete(AbstractGoTween tween)
	{
		StartNextTween();
	}
	...
	public void StartNextTween()
	{
		...
		GoTween nextTween = tweensToDo.Shift();
