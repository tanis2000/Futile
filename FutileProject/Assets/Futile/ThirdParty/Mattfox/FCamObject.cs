...
public class FCamObject : FContainer {
	...
	public void moveToPoint(Vector2 point, float time=0.0f) {
		...
		if (time == 0.0f) {
			...
			Go.to(this, time, new GoTweenConfig().floatProp("x", point.x).floatProp("y", point.y).setEaseType(GoEaseType.QuadInOut));
		}
	}
	...
	public void moveTo(FNode givenObject, float time=0.0f) 
	{
		...
		if (time == 0.0f) {
			...
			// tween to the given point
			Go.to(this, time, new GoTweenConfig().floatProp("x", givenObject.x).floatProp("y", givenObject.y).setEaseType(GoEaseType.QuadInOut));
		}
	}
