...
public class FTilemap : FContainer {
	...
	virtual public void Update() {
		if (clipToScreen && _clipNode != null) {
			// get position of _clipNode relative to this
			Vector2 relPos = StageToLocal(_clipNode.GetPosition());
			float xMin = relPos.x - clipWidth / 2 - _tileWidth * 0.75f;
			float yMin = relPos.y - clipHeight / 2 - _tileHeight * 0.75f;
			...
			_clipPos = new Vector2(Mathf.Round(xMin / _tileWidth) + _tileWidth, Mathf.Round(yMin / _tileHeight) + _tileHeight);
			
			float tileOffsetX = (Mathf.Floor(clipWidth / _tileWidth) + 2) * _tileWidth;
			float tileOffsetY = (Mathf.Floor(clipHeight / _tileHeight) + 2) * _tileHeight;
	...
	public void LoadText(string text, bool skipZero = true) {
		...
		//split on newline and ignore newline directly before eof and all other empty lines
	...
	public float getRight() {
		float returnX = 0;
		foreach (FNode node in _childNodes) {
			if (returnX < node.x + _tileWidth) {
				returnX = node.x + _tileWidth;
			}
		}
	...
	public FShader shader {
		...
		set {
			if (_shader != value) {
				...
				foreach (FNode tile in _childNodes) {
					((FFacetNode)tile).shader = _shader;
				}
			}
		}
	}
